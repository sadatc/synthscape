...
plotHistByModel <-function(model, dataFrameOrig, colName, fileName, showPercent=FALSE) {


	dataFrame <- dataFrameOrig[dataFrameOrig$MODEL==model &  dataFrameOrig$COMPLEXITY==3 
	&  dataFrameOrig$CLONES==10 &  dataFrameOrig$GRIDS==625 &  dataFrameOrig$RESOURCES==93 &  dataFrameOrig$DIFFICULTY=="r" ,]	
	print(summary(dataFrame))
	xAxisLabel <- getMeasurePrettyName(colName)
	pdf("/Users/sadat/Dropbox/research/dissertation/images/exp1/normality1.pdf", width = 6,height = 3, family="CMU Serif")
	...
	dataFrame <- dataFrameOrig[dataFrameOrig$MODEL==model &  dataFrameOrig$COMPLEXITY==3 
	 &  dataFrameOrig$GRIDS==625 &  dataFrameOrig$RESOURCES==93 &  dataFrameOrig$DIFFICULTY=="r" ,]	
	
	print(summary(dataFrame))
	xAxisLabel <- getMeasurePrettyName(colName)
	pdf("/Users/sadat/Dropbox/research/dissertation/images/exp1/normality2.pdf", width = 6,height = 3, family="CMU Serif")
	...
	dataFrame <- dataFrameOrig[dataFrameOrig$MODEL==model &  dataFrameOrig$COMPLEXITY==3
	  &  dataFrameOrig$DIFFICULTY=="r" ,]	
	
	print(summary(dataFrame))
	xAxisLabel <- getMeasurePrettyName(colName)
	pdf("/Users/sadat/Dropbox/research/dissertation/images/exp1/normality3.pdf", width = 6,height = 3, family="CMU Serif")
	...
	print(summary(dataFrame))
	xAxisLabel <- getMeasurePrettyName(colName)
	pdf("/Users/sadat/Dropbox/research/dissertation/images/exp1/normality4.pdf", width = 6,height = 3, family="CMU Serif")
...
plotHists <- function(expDataFrame) {
	# == plot general distributions ===
		...
		plotHistByModel("alife", expDataFrame,"CAPTURES_MEAN", "/Users/sadat/Dropbox/research/dissertation/images/exp1/test.pdf", TRUE)
		stop()
...
plotBoxPlot_M_I <-function(dataFrame, colName, fileName, showPercent=FALSE) {
	...
	if( showPercent==FALSE ) {
		print(
			...
			geom_boxplot(aes(fill=MODEL), notch=globalNotchValue) +
			facet_grid( POPULATION ~ INTERACTIONS, labeller=label_parsed) +
			ylab(yAxisLabel) +
