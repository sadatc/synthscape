...
doNormalityAnalysis <- function(df) {
	

	print("========== island ========")
	distTable <- data.frame()

	measures <- c("CAPTURES_MEAN","RATE_MOTION","RATE_COMMUNICATION", "RES_E2C_STEPS_MEAN" )
	counter <- 0

	for(model in unique(df$MODEL)) {
		print(model)
		for(interactions in unique(df$INTERACTIONS)) {
			for(compexity in unique(df$COMPLEXITY)) {
				for(clones in unique(df$CLONES)) {
					for(environment in unique(df$ENVIRONMENT))
								data <- df[
										df$MODEL==model & df$INTERACTIONS==interactions & 
										df$COMPLEXITY==compexity & df$CLONES==clones &
...
renameFactorValues <- function(dataFrame) {
	...
	# this creates the environments...
	envCounter <- 0		
	dataFrame$ENVIRONMENT <- 0
	dataFrame$ENVIRONMENT <- factor(dataFrame$ENVIRONMENT,c(1,2,3,4,5,6,7,8))
	for(resources in unique(dataFrame$RESOURCES)) {
		for(difficulty in unique(dataFrame$DIFFICULTY)) {
			print(paste(resources,difficulty,envCounter))
			envCounter <- envCounter +1 
			dataFrame$ENVIRONMENT[dataFrame$RESOURCES==resources & dataFrame$DIFFICULTY==difficulty] <- envCounter
		}
	}
