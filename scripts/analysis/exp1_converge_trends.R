		...
		for(species in getElements("SPECIES")) {
			for(interactions in getElements("INTERACTIONS")) {
				for(generations in 1:MAX_GENERATIONS) {
					CAPTURES_MEAN <- paste(model,species,interactions,generations,"CAPTURES_MEAN",sep=":")
					CAPTURES_MEAN <- getAccumulatedMean(CAPTURES_MEAN)

					CAPTURES_BEST_CASE <- paste(model,species,interactions,generations,"CAPTURES_BEST_CASE",sep=":")
					CAPTURES_BEST_CASE <- getAccumulatedMean(CAPTURES_BEST_CASE)

					RES_E2C_STEPS_MEAN <- paste(model,species,interactions,generations,"RES_E2C_STEPS_MEAN",sep=":")
					RES_E2C_STEPS_MEAN <- getAccumulatedMean(RES_E2C_STEPS_MEAN)

					RATE_COMMUNICATION <- paste(model,species,interactions,generations,"RATE_COMMUNICATION",sep=":")
