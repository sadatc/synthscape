...
public class Analyzer implements Constants {
	...
	public static void main(String[] args) throws Exception {
		...
		options.addOption(OptionBuilder.withArgName("dir").isRequired()
				.hasArg().withDescription("directory with all the csv files")
				.create("dir"));

		options.addOption(OptionBuilder.withArgName("outfile").isRequired()
				.hasArg().withDescription("full pathname to output file")
				.create("outfile"));
		...
		if (line.hasOption("outfile")
				&& line.getOptionValue("outfile").endsWith(".csv")) {
			outFileName = line.getOptionValue("outfile");
			File outFile = new File(outFileName);
			if (outFile.exists() && !outFile.isFile()) {
				D.p("ERROR: "
						+ outFileName
						+ " exists and is not a file; use a different filename!");
			} else if (outFile.exists() && outFile.isFile()) {
				D.p("WARNING: " + outFileName
						+ " already exists and will be overwritten!");
				outFile.delete();
			}

			String convergenceFileName = outFileName.replace(".csv",
					"_convergence_stats.csv");
			...
			if (line.hasOption("dir")) {
				...
				if (csvDirectory.exists()) {
					File[] csvFiles = csvDirectory
							.listFiles(new Analyzer.OnlyCSV());
					if (csvFiles.length > 0) {
						processCSVs(csvFiles);
						writeSummaryFile(outFile);
						writeConvergenceFile(convergenceFile);
					} else {
						D.p("ERROR: CSV directory named:" + directoryName
	...
	private static void writeConvergenceFile(File outFile) throws Exception {
		...
		BufferedWriter writer = new BufferedWriter(new FileWriter(
				outFile.getAbsoluteFile(), false), FILE_IO_BUFFER_SIZE);

		writer.write("experiment, csvfile, generations");
		writer.newLine();
		for (int i = 0; i < simGenerationCount.size(); i++) {
			writer.write((i + 1) + ", " + processedCsvFiles.get(i) + ", "
					+ simGenerationCount.get(i));
			writer.newLine();
		}
		...
		for (int rowNumber = 1; rowNumber <= maxRowNumber; rowNumber++) {
			...
			for (int i = 1; i < expectedFields.size(); i++) {
				...
				if (fieldRowStats.containsKey(expectedFields.get(i))) {
					LinkedHashMap<Integer, SummaryStatistics> rowFieldStats = fieldRowStats
							.get(expectedFields.get(i));
			...
			if (expectedFields.size() == 0) {
				...
				for (String fieldName : fieldNames) {
					...
					if (!fieldName.equals(expectedFields.get(counter))) {
						D.p("ERROR:" + fieldName
								+ " didn't match expected field:"
								+ expectedFields.get(counter));
						result = false;
						break;
					}
					counter++;
			...
			while ((row = reader.readLine()) != null) {
				if (rowNumber == 0) {
					...
					if (!processFields(row)) {
						D.p("ERROR while processing " + csvFile.getName()
								+ " ... moving on...");
						break;
					}
					rowNumber++;
	...
	private static void updateFieldRowStats(String fieldName, int rowNumber,
			double doubleValue) {
