...
public final class EvolutionEngine implements Constants {
	...
	public EvolutionEngine(EmbodiedAgent ownerAgent, AgentFactory agentFactory,
			Species species) throws Exception {
		this(ownerAgent, agentFactory, species, EE_DEF_PERCENT_TOP,
				EE_DEF_PERCENT_TOP_X_TOP, EE_DEF_PERCENT_TOP_MUTANT,
				EE_DEF_PERCENT_TOP_X_BOTTOM, EE_DEF_PERCENT_BOTTOM,
				EE_DEF_PERCENT_BOTTOM_MUTANT, EE_DEF_PERCENT_BOTTOM_X_BOTTOM,
				EE_DEF_PERCENT_RANDOM, EE_DEF_MAX_MUTATION_RATE,
	 ...
	 * @throws Exception
	 */
	public EvolutionEngine(AgentFactory agentFactory, Species species)
			throws Exception {
		this(null, agentFactory, species, EE_DEF_PERCENT_TOP,
				EE_DEF_PERCENT_TOP_X_TOP, EE_DEF_PERCENT_TOP_MUTANT,
				EE_DEF_PERCENT_TOP_X_BOTTOM, EE_DEF_PERCENT_BOTTOM,
				EE_DEF_PERCENT_BOTTOM_MUTANT, EE_DEF_PERCENT_BOTTOM_X_BOTTOM,
				EE_DEF_PERCENT_RANDOM, EE_DEF_MAX_MUTATION_RATE,
	 ...
	 */
	public EvolutionEngine(AgentFactory agentFactory, Species species,
			int genePoolSize) throws Exception {
		this(null, agentFactory, species, EE_DEF_PERCENT_TOP,
				EE_DEF_PERCENT_TOP_X_TOP, EE_DEF_PERCENT_TOP_MUTANT,
				EE_DEF_PERCENT_TOP_X_BOTTOM, EE_DEF_PERCENT_BOTTOM,
				EE_DEF_PERCENT_BOTTOM_MUTANT, EE_DEF_PERCENT_BOTTOM_X_BOTTOM,
	 ...
	 * @throws Exception
	 */
	public EvolutionEngine(EmbodiedAgent engineOwnerAgent,
			AgentFactory agentFactory, Species species, double percentTop,
			...
			double percentRandom, double maxMutationRate,
		...
		if (aTop <= 0) {
			throw new Exception("aTop is too low. Most likely, genePool="
					+ genePoolSize + ", is too small");
		}
		aTopXTop = (int) ((double) genePoolSize * percentTopXTop);
		if (aTopXTop <= 0) {
			throw new Exception("aTopXTop is too low. Most likely, genePool="
					+ genePoolSize + ", is too small");
		}
		aTopMutants = (int) ((double) genePoolSize * percentTopMutant);
		if (aTopMutants <= 0) {
			throw new Exception(
					"aTopMutants is too low. Most likely, genePool="
							+ genePoolSize + ", is too small");
		}
		aTopXBottom = (int) ((double) genePoolSize * percentTopXBottom);
		if (aTopXBottom <= 0) {
			throw new Exception(
					"aTopXBottom is too low. Most likely, genePool="
							+ genePoolSize + ", is too small");
		}
		aBottom = (int) ((double) genePoolSize * percentBottom);
		if (aBottom <= 0) {
			throw new Exception("aBottom is too low. Most likely, genePool="
					+ genePoolSize + ", is too small");
		}
		aBottomMutants = (int) ((double) genePoolSize * percentBottomMutant);
		if (aBottomMutants <= 0) {
			throw new Exception(
					"aBottomMutants is too low. Most likely, genePool="
							+ genePoolSize + ", is too small");
		}
		aBottomXBottom = (int) ((double) genePoolSize * percentBottomXBottom);
		if (aBottomXBottom <= 0) {
			throw new Exception(
					"aBottomXBottom is too low. Most likely, genePool="
							+ genePoolSize + ", is too small");
		}
		aRandom = (int) ((double) genePoolSize * percentRandom);
		if (aRandom <= 0) {
			throw new Exception("aRandom is too low. Most likely, genePool="
					+ genePoolSize + ", is too small");
		}
	...
	private void computeFitnessStats() {
		...
		if (engineOwnerAgent != null) {
			DescriptiveStatistics.copy(fitnessStats,
					engineOwnerAgent.fitnessStats);
		}
	}
	...
	private void printActiveBufferStats() {
		...
		if (generationCounter > 0) {
			if (settings.EVOLUTIONARY_MODEL == EvolutionaryModel.EMBODIED_MODEL
				...
				logger.info("Gen: " + generationCounter + " Sp: "
						+ species.toString() + " Cap: " + captures + " AvgFit:"
						+ fitnessStats.getMean() + " PoolComp: "
				...
				if (settings.lastLoggedGeneration != generationCounter) {
					...
					logger.info("Gen: " + generationCounter + " Sp: "
							+ species.toString() + " Cap: "
							+ settings.lastReportedCaptures + " PoolComp: "
							+ getPoolCompositionString());
	...
	public final void generateNextGeneration(
		...
		swapBufferElements(activeBuffer, offspringBuffer, genePoolSize
