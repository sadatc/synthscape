...
public abstract class Simulation extends SimState implements Constants {
	...
	resourceCaptureGoal = (int) ((double) numberOfResources
		* settings.PERC_RESOURCE_CAPTURE_GOAL);
	...
	this.resourceCaptureGoal = (int) ((double) numberOfResources
		* settings.PERC_RESOURCE_CAPTURE_GOAL);
	...
	GridUtils.set(collisionGrid, settings.PRIMARY_COLLECTION_SITE_X,
		settings.PRIMARY_COLLECTION_SITE_Y, true);

	collectionSiteList.add(
		new Int2D(settings.PRIMARY_COLLECTION_SITE_X, settings.PRIMARY_COLLECTION_SITE_Y));
    }
	...
	if (agentNum == 0) {
	    ...
	    if (!GridUtils.gridHasAnObjectAt(collisionGrid, agentGrid.stx(pX),
		    agentGrid.sty(pY - 1))) {
	    ...
	    else if (!GridUtils.gridHasAnObjectAt(collisionGrid, agentGrid.stx(pX - 1),
		    agentGrid.sty(pY - 1))) {
		// NW
	    ...
	    else if (!GridUtils.gridHasAnObjectAt(collisionGrid, agentGrid.stx(pX),
		    agentGrid.sty(pY + 1))) {
		// S
		randomY = agentGrid.sty(pY + 1);
	    }
