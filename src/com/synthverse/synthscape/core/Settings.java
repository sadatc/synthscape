...
public class Settings implements Constants {
	...
	options.addOption(OptionBuilder.withArgName("iquality").hasArg()
		.withDescription("interaction quality (highest, high, medium, low/poor) [highest]")
		.create("iquality"));
	...
	options.addOption(OptionBuilder.withArgName("job_name").hasArg()
		.withDescription("job name [" + JOB_NAME + "]").create("job_name"));
	...
	try {
		...
		if (dir.exists() && dir.isDirectory()) {
		    ...
		    if (!dir.mkdir()) {
			throw new ParseException("Unable to create: " + DATA_DIR
